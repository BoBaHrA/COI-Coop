param(
    ...
    [string]$OutputPath = (Join-Path $PSScriptRoot "blueprint-api.txt")
)
...
$matches = @($allTypes | Where-Object {
    $n = $_.FullName
    $n -and (
        $n.IndexOf("Blueprint", [System.StringComparison]::OrdinalIgnoreCase) -ge 0 -or
        $n.IndexOf("CopyPaste", [System.StringComparison]::OrdinalIgnoreCase) -ge 0 -or
        $n.IndexOf("StaticEntityMassPlacer", [System.StringComparison]::OrdinalIgnoreCase) -ge 0
    )
...
$lines.Add("Captain of Industry blueprint placement API metadata")
...
Write-Host "Blueprint API metadata written to: $OutputPath"
